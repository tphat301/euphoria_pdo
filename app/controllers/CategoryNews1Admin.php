...
class CategoryNews1Admin extends Controllers
{
  public $data = [];
  public $error = [];
  public $success = [];
  ...
  public function index()
  {
    ...
    @$type = $this->newsType;
    @$url = $this->urlBase . "admin/category_news1/index";
    @$numberPerPage = $this->numberPage;
    $categoryNewsModel = $this->model->models($this->categoryNewsModel);
    $categoryNews = $categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `type` = '{$type}' AND `deleted_at` IS NULL  ORDER BY num,id DESC");
    $softDelete = $categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `type` = '{$type}' AND `deleted_at` IS NOT NULL");
    $numberRow = count($categoryNews);
    ...
    $newsPaginate = $categoryNewsModel->all('*', $this->tableCategoryNews, $where . "`level` = 1 AND `type` = '{$type}' AND `deleted_at` IS NULL ORDER BY num,id DESC" . $limit);
    if ($newsPaginate) {
  ...
  public function show()
  {
    ...
    $catNewsDetail = Functions::rawOne($categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `id` = $id AND `type` = '{$type}' AND `deleted_at` IS NULL LIMIT 0,1"));
  ...
  public function ajaxStatus()
  {
    ...
    @$type = $this->newsType;
    $categoryNewsModel = $this->model->models($this->categoryNewsModel);
    $statusCategoryNews = Functions::rawOne($categoryNewsModel->all('status', $table, "`level` = 1 AND `id` = '{$id}' AND `type` = '{$type}'"))['status'];
    ...
    $statusCategoryNewStr = implode(',', $statusCategoryNewsArray);
    $categoryNewsModel->update($table, array('status' => $statusCategoryNewStr), "`level` = 1 AND `id`='{$id}' AND `type` = '{$type}'");
  }
  ...
  public function ajaxNumber()
  {
    ...
    @$type = $this->newsType;
    $categoryNewsModel = $this->model->models($this->categoryNewsModel);
    $categoryNewsModel->update($table, array('num' => $num), "`level` = 1 AND `id`='{$id}' AND `type` = '{$type}'");
  }
  ...
  public function deleteAll()
  {
    if ($_POST['checkitem']) {
      @$type = $this->newsType;
      $uploadCategoryNews = "upload/category_news1/";
      $categoryNewsModel = $this->model->models($this->categoryNewsModel);
      $seoModel = $this->model->models($this->seoModel);
      ...
      $categoryNewsByListId = $categoryNewsModel->all("*", $this->tableCategoryNews, "`level` = 1 AND `id` IN ($listCheck) AND `type` = '{$type}'");
      ...
      $categoryNewsModel->destroy($this->tableCategoryNews, "`level` = 1 AND `id` IN ($listCheck) AND `type` = '{$type}'");
      Functions::transfer("Xóa dữ liệu", "success", $this->urlBase . "admin/category_news1/index");
    } else {
      Functions::transfer("Xóa dữ liệu", "danger", $this->urlBase . "admin/category_news1/index");
    }
  }
  ...
  public function softDeleteIndex()
  {
    @$type = $this->newsType;
    @$numberPerPage = $this->numberPage;
    @$url = $this->urlBase . "admin/category_news1/soft";
    $categoryNewsModel = $this->model->models($this->categoryNewsModel);
    $data = $categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `type` = '{$type}' AND `deleted_at` IS NOT NULL");
    $numberRow = count($data);
    $totalPage = ceil($numberRow / $numberPerPage);
    $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
    $numberStart = ($page - 1) * $numberPerPage;
    $paginate = Functions::Paginate($page, $totalPage, $url);
    $categoryNewsPaginate = $categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `type` = '{$type}' AND `deleted_at` IS NOT NULL ORDER BY num,id DESC LIMIT $numberStart,$numberPerPage");
    $this->data['seoTitle'] = $this->seoTitleTrash;
    $this->data['module'] = $this->nameModuleSoftDeleteNews;
    $this->data['action'] = 'category_news1';
    $this->data['paginate'] = $paginate;
  ...
  public function softDelete()
  {
    ...
    $categoryNewsModel = $this->model->models($this->categoryNewsModel);
    if ($id) {
      $deleteAt = Functions::rawOne($categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `id` = '{$id}' AND `deleted_at` IS NULL LIMIT 0,1"));
      if (Functions::checkData($deleteAt)) {
        $categoryNewsModel->edit($this->tableCategoryNews, ['deleted_at' => time()], "`level` = 1 AND `id`='{$id}' AND `type` = '{$type}'");
        Functions::transfer("Xóa dữ liệu", "success", $this->urlBase . "admin/category_news1/index");
      } else {
        Functions::transfer("Xóa dữ liệu", "danger", $this->urlBase . "admin/category_news1/index");
      }
  ...
  public function softDeleteAll()
  {
    if ($_POST['checkitem']) {
      ...
      @$type = $this->newsType;
      $categoryNewsModel = $this->model->models($this->categoryNewsModel);
      $dataDeleted = $categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `id` IN ($listCheck)  AND `type` = '{$type}' AND  `deleted_at` IS NULL");
      if (Functions::checkData($dataDeleted)) {
        $categoryNewsModel->edit($this->tableCategoryNews, ['deleted_at' => time()], "`level` = 1 AND `id` IN ($listCheck) AND `type` = '{$type}'");
        Functions::transfer("Xóa dữ liệu", "success", $this->urlBase . "admin/category_news1/index");
      } else {
        Functions::transfer("Xóa dữ liệu", "danger", $this->urlBase . "admin/category_news1/index");
      }
  ...
  public function restore()
  {
    ...
    $categoryNewsModel = $this->model->models($this->categoryNewsModel);
    if ($id) {
      $deleteAt = $categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `id` = $id AND `type` = '{$type}' AND `deleted_at` IS NOT NULL LIMIT 0,1");
      if ($deleteAt) {
        $categoryNewsModel->edit($this->tableCategoryNews, ['deleted_at' => null], "`level` = 1 AND `id` = '{$id}' AND `type` = '{$type}'");
        Functions::transfer("Khôi phục dữ liệu", "success", $this->urlBase . "admin/category_news1/index");
      } else {
        Functions::transfer("Khôi phục dữ liệu", "danger", $this->urlBase . "admin/category_news1/index");
      }
    }
  }
  ...
  public function restoreAll()
  {
    if ($_POST['checkitem']) {
      ...
      @$listCheck = implode(',', array_values($_POST['checkitem']));
      $categoryNewsModel = $this->model->models($this->categoryNewsModel);
      $dataDeleted = $categoryNewsModel->all("*", $this->tableCategoryNews, "`level` = 1 AND `type` = '{$type}' AND `id` IN ($listCheck) AND `deleted_at` IS NOT NULL");
      if (Functions::checkData($dataDeleted)) {
        $categoryNewsModel->edit($this->tableCategoryNews, ['deleted_at' => null], "`level` = 1 AND `id` IN ($listCheck) AND `type` = '{$type}'");
        Functions::transfer("Khôi phục dữ liệu", "success", $this->urlBase . "admin/category_news1/index");
      } else {
        Functions::transfer("Khôi phục dữ liệu", "danger", $this->urlBase . "admin/category_news1/index");
      }
  ...
  public function deletePhoto()
  {
    ...
    if ($photo1 && !empty($id)) {
      ...
      $categoryNewsModel->edit($this->tableCategoryNews, ["photo1" => null], "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}'");
      Functions::transfer("Xóa ảnh", "success", $this->urlBase . "admin/category_news1/show?id=" . $id);
    } elseif ($photo2 && !empty($id)) {
      $filename = $uploadCategoryNews . $photo2;
      if (file_exists($filename) && !empty($photo2)) unlink($filename);
      $categoryNewsModel->edit($this->tableCategoryNews, ["photo2" => null], "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}'");
      Functions::transfer("Xóa ảnh", "success", $this->urlBase . "admin/category_news1/show?id=" . $id);
    } elseif ($photo3 && !empty($id)) {
      $filename = $uploadCategoryNews . $photo3;
      if (file_exists($filename) && !empty($photo3)) unlink($filename);
      $categoryNewsModel->edit($this->tableCategoryNews, ["photo3" => null], "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}'");
      Functions::transfer("Xóa ảnh", "success", $this->urlBase . "admin/category_news1/show?id=" . $id);
    } elseif ($photo4 && !empty($id)) {
      $filename = $uploadCategoryNews . $photo4;
      if (file_exists($filename) && !empty($photo4)) unlink($filename);
      $categoryNewsModel->edit($this->tableCategoryNews, ["photo4" => null], "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}'");
      Functions::transfer("Xóa ảnh", "success", $this->urlBase . "admin/category_news1/show?id=" . $id);
    } else {
      Functions::redirect($this->urlBase . "admin/category_news1/show?id=" . $id);
    }
  }
  ...
  public function destroy()
  {
    ...
    $seoModel = $this->model->models($this->seoModel);
    $uploadCategoryNews = "upload/category_news1/";
    $categoryNewsDetail = Functions::rawOne($categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `type` = '{$type}' AND `hash` = '{$hash}' AND `id` = '{$id}' LIMIT 0,1"));
    ...
    if (file_exists($filenameCategoryNews3) && !empty($photo3)) unlink($filenameCategoryNews3);
    if (file_exists($filenameCategoryNews4) && !empty($photo4)) unlink($filenameCategoryNews4);
    $categoryNewsModel->destroy($this->tableCategoryNews, "`level` = 1 AND `type` = '{$type}' AND `id`='{$id}'");
    Functions::transfer("Xóa dữ liệu", "success", $this->urlBase . "admin/category_news1/index");
  }

  /* Duplicate category news row */
  public function duplicate()
  {
    ...
    $categoryNewsModel = $this->model->models($this->categoryNewsModel);
    $categoryNewsDetail = Functions::rawOne($categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}' limit 0,1"));
    ...
    Functions::transfer("Nhân bản dữ liệu", "success", $this->urlBase . "admin/category_news1/index");
  }

  /* Hanlde save && save here && update category news */
  public function stored()
  {
    ...
    $categoryNewsModel = $this->model->models($this->categoryNewsModel);
    $seoModel = $this->model->models($this->seoModel);
    $hashString = Functions::rawOne($categoryNewsModel->all('`hash`', $this->tableCategoryNews, "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}' LIMIT 0,1"));
    ...
    if ($id) {
      ...
      if (isset($_POST['save-here'])) {
        ...
        if (!empty($_POST['slug'])) {
          ...
          $slugs = $categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `type` = '{$type}' AND `id` <> $id AND `slug` = '{$slugCondition}'");
        ...
        if (empty($this->error)) {
          ...
          if (Functions::hasFile("photo1")) {
            $photo1 = Functions::uploadFile("photo1", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "upload/category_news1/");
          } else {
          ...
          if (Functions::hasFile("photo2")) {
            $photo2 = Functions::uploadFile("photo2", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "upload/category_news1/");
          } else {
          ...
          if (Functions::hasFile("photo3")) {
            $photo3 = Functions::uploadFile("photo3", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "upload/category_news1/");
          } else {
          ...
          if (Functions::hasFile("photo4")) {
            $photo4 = Functions::uploadFile("photo4", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "upload/category_news1/");
          } else {
          ...
          $categoryNewsModel->edit($this->tableCategoryNews, $d, "`level` = 1 AND `id`='{$id}'");
          ...
          Functions::transfer("Cập nhật dữ liệu", "success", $this->urlBase . "admin/category_news1/show?id=" . $id);
        } else {
          foreach ($this->error as $k => $v) {
            Flash::set($k, $v, 'danger');
          }
          Functions::transfer("Cập nhật dữ liệu", "danger", $this->urlBase . "admin/category_news1/show?id=" . $id);
        }
      }
      ...
      if (isset($_POST['update'])) {
        ...
        if (empty($this->error)) {
          ...
          Functions::transfer("Cập nhật dữ liệu", "success", $this->urlBase . "admin/category_news1/index");
        } else {
          foreach ($this->error as $k => $v) {
            Flash::set($k, $v, 'danger');
          }
          Functions::transfer("Cập nhật dữ liệu", "danger", $this->urlBase . "admin/category_news1/index");
        }
      }
    } else {
      /* SAVE DATA */
      if (isset($_POST['save'])) {
        ...
        if (!empty($_POST['slug'])) {
          $slugCondition = htmlspecialchars($_POST['slug']);
          $slugs = $categoryNewsModel->all('*', $this->tableCategoryNews, "`level` = 1 AND `type` = '{$type}' AND `slug` = '{$slugCondition}'");
          if (Functions::checkData($slugs)) {
            $this->success['slug'] = htmlspecialchars($_POST['slug']);
            foreach ($this->success as $k => $v) {
              Flash::set($k, $v, 'success');
        ...
        if (empty($this->error)) {
          ...
          $categoryNewsModel->create($this->tableCategoryNews, $d);
          Functions::transfer("Thêm dữ liệu", "success", $this->urlBase . "admin/category_news1/index");
        } else {
          foreach ($this->error as $k => $v) {
            Flash::set($k, $v, 'danger');
          }
          Functions::transfer("Thêm dữ liệu", "danger", $this->urlBase . "admin/category_news1/create");
        }
      }
    }
  }
