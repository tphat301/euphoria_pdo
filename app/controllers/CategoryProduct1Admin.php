...
class CategoryProduct1Admin extends Controllers
{
  public $data = [];
  public $error = [];
  public $success = [];
  ...
  public function __construct()
  {
    ...
    $this->numberPage = Configurations::configurationsBackEnd()['category_product1']['num_per_page'];
    $this->seoTitleIndex = "Danh sách danh mục cấp 1";
    $this->seoTitleCreate = "Thêm danh mục cấp 1";
    $this->seoTitleTrash = "Thùng rác";
    $this->tableSeo = "seo";
    $this->nameModuleCategoryProduct = "category_product1";
    $this->nameModuleSoftDeleteProduct = "soft_delete_product";
    $this->categoryProductModel = "CategoryProductAdmin";
    $this->seoModel = "SeoAdmin";
    $this->tableCategoryProduct = Configurations::configurationsBackEnd()['category_product1']['table'];
  }
  ...
  public function index()
  {
    ...
    @$url = $this->urlBase . "admin/category_product1/index";
    @$numberPerPage = $this->numberPage;
    $categoryProductModel = $this->model->models($this->categoryProductModel);
    $categoryProduct = $categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `type` = '{$type}' AND `deleted_at` IS NULL  ORDER BY num,id DESC");
    $softDelete = $categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `type` = '{$type}' AND `deleted_at` IS NOT NULL");
    $numberRow = count($categoryProduct);
    $totalPage = ceil($numberRow / $numberPerPage);
    $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
    $numberStart = ($page - 1) * $numberPerPage;
    $limit = " LIMIT $numberStart,$numberPerPage";
    $where = "";
    if ($keyword) $where .= "`title` LIKE '%{$keyword}%' AND ";
    $productPaginate = $categoryProductModel->all('*', $this->tableCategoryProduct, $where . "`level` = 1 AND `type` = '{$type}' AND `deleted_at` IS NULL ORDER BY num,id DESC" . $limit);
    if ($productPaginate) {
  ...
  public function show()
  {
    ...
    $catProductDetail = Functions::rawOne($categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `id` = $id AND `type` = '{$type}' AND `deleted_at` IS NULL LIMIT 0,1"));
  ...
  public function ajaxStatus()
  {
    ...
    @$type = $this->productType;
    $categoryProductModel = $this->model->models($this->categoryProductModel);
    $statusCategoryProduct = Functions::rawOne($categoryProductModel->all('status', $table, "`level` = 1 AND `id` = '{$id}' AND `type` = '{$type}'"))['status'];
    ...
    $categoryProductModel->update($table, array('status' => $statusCategoryProductStr), "`level` = 1 AND `id`='{$id}' AND `type` = '{$type}'");
  }
  ...
  public function ajaxNumber()
  {
    ...
    @$type = $this->productType;
    $categoryProductModel = $this->model->models($this->categoryProductModel);
    $categoryProductModel->update($table, array('num' => $num), "`level` = 1 AND `id`='{$id}' AND `type` = '{$type}'");
  }
  ...
  public function deleteAll()
  {
    if ($_POST['checkitem']) {
      @$type = $this->productType;
      $uploadCategoryProduct = "upload/category_product1/";
      $categoryProductModel = $this->model->models($this->categoryProductModel);
      $seoModel = $this->model->models($this->seoModel);
      ...
      $categoryProductByListId = $categoryProductModel->all("*", $this->tableCategoryProduct, "`level` = 1 AND `id` IN ($listCheck) AND `type` = '{$type}'");
      ...
      $categoryProductModel->destroy($this->tableCategoryProduct, "`level` = 1 AND `id` IN ($listCheck) AND `type` = '{$type}'");
      Functions::transfer("Xóa dữ liệu", "success", $this->urlBase . "admin/category_product1/index");
    } else {
      Functions::transfer("Xóa dữ liệu", "danger", $this->urlBase . "admin/category_product1/index");
    }
  }
  ...
  public function softDeleteIndex()
  {
    @$type = $this->productType;
    @$numberPerPage = $this->numberPage;
    @$url = $this->urlBase . "admin/category_product1/soft";
    $categoryProductModel = $this->model->models($this->categoryProductModel);
    $data = $categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `type` = '{$type}' AND `deleted_at` IS NOT NULL");
    $numberRow = count($data);
    $totalPage = ceil($numberRow / $numberPerPage);
    $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
    $numberStart = ($page - 1) * $numberPerPage;
    $paginate = Functions::Paginate($page, $totalPage, $url);
    $categoryProductPaginate = $categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `type` = '{$type}' AND `deleted_at` IS NOT NULL ORDER BY num,id DESC LIMIT $numberStart,$numberPerPage");
    $this->data['seoTitle'] = "Thùng rác";
    $this->data['module'] = $this->nameModuleSoftDeleteProduct;
    $this->data['action'] = 'category_product1';
    $this->data['paginate'] = $paginate;
  ...
  public function softDelete()
  {
    ...
    $categoryProductModel = $this->model->models($this->categoryProductModel);
    if ($id) {
      $deleteAt = Functions::rawOne($categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `id` = '{$id}' AND `deleted_at` IS NULL LIMIT 0,1"));
      if (Functions::checkData($deleteAt)) {
        $categoryProductModel->edit($this->tableCategoryProduct, ['deleted_at' => time()], "`level` = 1 AND `id`='{$id}' AND `type` = '{$type}'");
        Functions::transfer("Xóa dữ liệu", "success", $this->urlBase . "admin/category_product1/index");
      } else {
        Functions::transfer("Xóa dữ liệu", "danger", $this->urlBase . "admin/category_product1/index");
      }
  ...
  public function softDeleteAll()
  {
    if ($_POST['checkitem']) {
      ...
      @$type = $this->productType;
      $categoryProductModel = $this->model->models($this->categoryProductModel);
      $dataDeleted = $categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `id` IN ($listCheck)  AND `type` = '{$type}' AND  `deleted_at` IS NULL");
      if (Functions::checkData($dataDeleted)) {
        $categoryProductModel->edit($this->tableCategoryProduct, ['deleted_at' => time()], "`level` = 1 AND `id` IN ($listCheck) AND `type` = '{$type}'");
        Functions::transfer("Xóa dữ liệu", "success", $this->urlBase . "admin/category_product1/index");
      } else {
        Functions::transfer("Xóa dữ liệu", "danger", $this->urlBase . "admin/category_product1/index");
      }
  ...
  public function restore()
  {
    ...
    $categoryProductModel = $this->model->models($this->categoryProductModel);
    if ($id) {
      $deleteAt = $categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `id` = $id AND `type` = '{$type}' AND `deleted_at` IS NOT NULL LIMIT 0,1");
      if ($deleteAt) {
        $categoryProductModel->edit($this->tableCategoryProduct, ['deleted_at' => null], "`level` = 1 AND `id` = '{$id}' AND `type` = '{$type}'");
        Functions::transfer("Khôi phục dữ liệu", "success", $this->urlBase . "admin/category_product1/index");
      } else {
        Functions::transfer("Khôi phục dữ liệu", "danger", $this->urlBase . "admin/category_product1/index");
      }
    }
  }
  ...
  public function restoreAll()
  {
    if ($_POST['checkitem']) {
      ...
      @$listCheck = implode(',', array_values($_POST['checkitem']));
      $categoryProductModel = $this->model->models($this->categoryProductModel);
      $dataDeleted = $categoryProductModel->all("*", $this->tableCategoryProduct, "`level` = 1 AND `type` = '{$type}' AND `id` IN ($listCheck) AND `deleted_at` IS NOT NULL");
      if (Functions::checkData($dataDeleted)) {
        $categoryProductModel->edit($this->tableCategoryProduct, ['deleted_at' => null], "`level` = 1 AND `id` IN ($listCheck) AND `type` = '{$type}'");
        Functions::transfer("Khôi phục dữ liệu", "success", $this->urlBase . "admin/category_product1/index");
      } else {
        Functions::transfer("Khôi phục dữ liệu", "danger", $this->urlBase . "admin/category_product1/index");
      }
  ...
  public function deletePhoto()
  {
    ...
    if ($photo1 && !empty($id)) {
      ...
      $categoryProductModel->edit($this->tableCategoryProduct, ["photo1" => null], "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}'");
      Functions::transfer("Xóa ảnh", "success", $this->urlBase . "admin/category_product1/show?id=" . $id);
    } elseif ($photo2 && !empty($id)) {
      $filename = $uploadCategoryProduct . $photo2;
      if (file_exists($filename) && !empty($photo2)) unlink($filename);
      $categoryProductModel->edit($this->tableCategoryProduct, ["photo2" => null], "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}'");
      Functions::transfer("Xóa ảnh", "success", $this->urlBase . "admin/category_product1/show?id=" . $id);
    } elseif ($photo3 && !empty($id)) {
      $filename = $uploadCategoryProduct . $photo3;
      if (file_exists($filename) && !empty($photo3)) unlink($filename);
      $categoryProductModel->edit($this->tableCategoryProduct, ["photo3" => null], "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}'");
      Functions::transfer("Xóa ảnh", "success", $this->urlBase . "admin/category_product1/show?id=" . $id);
    } elseif ($photo4 && !empty($id)) {
      $filename = $uploadCategoryProduct . $photo4;
      if (file_exists($filename) && !empty($photo4)) unlink($filename);
      $categoryProductModel->edit($this->tableCategoryProduct, ["photo4" => null], "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}'");
      Functions::transfer("Xóa ảnh", "success", $this->urlBase . "admin/category_product1/show?id=" . $id);
    } else {
      Functions::redirect($this->urlBase . "admin/category_product1/show?id=" . $id);
    }
  }
  ...
  public function destroy()
  {
    ...
    $seoModel = $this->model->models($this->seoModel);
    $uploadCategoryProduct = "upload/category_product1/";
    $categoryProductDetail = Functions::rawOne($categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `type` = '{$type}' AND `hash` = '{$hash}' AND `id` = '{$id}' LIMIT 0,1"));
    ...
    if (file_exists($filenameCategoryProduct3) && !empty($photo3)) unlink($filenameCategoryProduct3);
    if (file_exists($filenameCategoryProduct4) && !empty($photo4)) unlink($filenameCategoryProduct4);
    $categoryProductModel->destroy($this->tableCategoryProduct, "`level` = 1 AND `type` = '{$type}' AND `id`='{$id}'");
    Functions::transfer("Xóa dữ liệu", "success", $this->urlBase . "admin/category_product1/index");
  }
  ...
  public function duplicate()
  {
    ...
    $categoryProductModel = $this->model->models($this->categoryProductModel);
    $categoryProductDetail = Functions::rawOne($categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}' limit 0,1"));
    ...
    Functions::transfer("Nhân bản dữ liệu", "success", $this->urlBase . "admin/category_product1/index");
  }
  ...
  public function stored()
  {
    ...
    $categoryProductModel = $this->model->models($this->categoryProductModel);
    $seoModel = $this->model->models($this->seoModel);
    $hashString = Functions::rawOne($categoryProductModel->all('`hash`', $this->tableCategoryProduct, "`level` = 1 AND `type` = '{$type}' AND `id` = '{$id}' LIMIT 0,1"));
    ...
    if ($id) {
      ...
      if (isset($_POST['save-here'])) {
        ...
        if (!empty($_POST['slug'])) {
          ...
          $slugs = $categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `type` = '{$type}' AND `id` <> $id AND `slug` = '{$slugCondition}'");
        ...
        if (empty($this->error)) {
          ...
          if (Functions::hasFile("photo1")) {
            $photo1 = Functions::uploadFile("photo1", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "upload/category_product1/");
          } else {
          ...
          if (Functions::hasFile("photo2")) {
            $photo2 = Functions::uploadFile("photo2", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "upload/category_product1/");
          } else {
          ...
          if (Functions::hasFile("photo3")) {
            $photo3 = Functions::uploadFile("photo3", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "upload/category_product1/");
          } else {
          ...
          if (Functions::hasFile("photo4")) {
            $photo4 = Functions::uploadFile("photo4", array('png', 'jpg', 'jpeg', 'gif', '.webp'), "upload/category_product1/");
          } else {
          ...
          $categoryProductModel->edit($this->tableCategoryProduct, $d, "`level` = 1 AND `id`='{$id}'");
          ...
          Functions::transfer("Cập nhật dữ liệu", "success", $this->urlBase . "admin/category_product1/show?id=" . $id);
        } else {
          foreach ($this->error as $k => $v) {
            Flash::set($k, $v, 'danger');
          }
          Functions::transfer("Cập nhật dữ liệu", "danger", $this->urlBase . "admin/category_product1/show?id=" . $id);
        }
      }
      ...
      if (isset($_POST['update'])) {
        ...
        if (empty($this->error)) {
          ...
          Functions::transfer("Cập nhật dữ liệu", "success", $this->urlBase . "admin/category_product1/index");
        } else {
          foreach ($this->error as $k => $v) {
            Flash::set($k, $v, 'danger');
          }
          Functions::transfer("Cập nhật dữ liệu", "danger", $this->urlBase . "admin/category_product1/index");
        }
      }
    } else {
      /* SAVE DATA */
      if (isset($_POST['save'])) {
        ...
        if (!empty($_POST['slug'])) {
          $slugCondition = htmlspecialchars($_POST['slug']);
          $slugs = $categoryProductModel->all('*', $this->tableCategoryProduct, "`level` = 1 AND `type` = '{$type}' AND `slug` = '{$slugCondition}'");
          if (Functions::checkData($slugs)) {
            $this->success['slug'] = htmlspecialchars($_POST['slug']);
            foreach ($this->success as $k => $v) {
              Flash::set($k, $v, 'success');
        ...
        if (empty($this->error)) {
          ...
          $categoryProductModel->create($this->tableCategoryProduct, $d);
          Functions::transfer("Thêm dữ liệu", "success", $this->urlBase . "admin/category_product1/index");
        } else {
          foreach ($this->error as $k => $v) {
            Flash::set($k, $v, 'danger');
          }
          Functions::transfer("Thêm dữ liệu", "danger", $this->urlBase . "admin/category_product1/create");
        }
      }
    }
  }
