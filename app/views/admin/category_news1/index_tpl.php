  ...
  <section class="content">
    <form action="" class="form-product-list" method="POST">
      <div class="card-footer text-sm sticky-top">
        ...
        <?php if (count($news) !== 0 && isset($configAdmin['category_news1']['soft_delete']) && $configAdmin['category_news1']['soft_delete'] === true) { ?>
          <a class="btn btn-sm bg-gradient-danger text-white delete-all" id="delete-all" href="javascript:void()" title="Xóa tất cả" data-url="<?= $configBase['baseUrl'] . "admin/category_news1/soft_delete_all" ?>"><i class="far fa-trash-alt mr-2"></i>Xóa tất cả</a>
        <?php } else { ?>
          <a class="btn btn-sm bg-gradient-danger text-white delete-all" id="delete-all" href="javascript:void()" title="Xóa tất cả" data-url="<?= $configBase['baseUrl'] . "admin/category_news1/delete_all" ?>"><i class="far fa-trash-alt mr-2"></i>Xóa tất cả</a>
        <?php } ?>

        <?php if (isset($configAdmin['category_news1']['soft_delete']) && $configAdmin['category_news1']['soft_delete'] === true) { ?>
          <a class="btn btn-sm bg-gradient-warning text-dark" id="trash-view" href="<?= $configBase['baseUrl'] . 'admin/category_news1/soft' ?>" title="Xóa tất cả"><i class="far fa-trash-alt mr-2"></i>Thùng rác (<?= isset($softDelete) ? count($softDelete) : 0 ?>)</a>
        <?php } ?>

        <div class="form-inline form-search d-inline-block align-middle ml-3">
          <div class="input-group input-group-sm">
            <input class="form-control form-control-navbar text-sm keyword" type="text" placeholder="Tìm kiếm" aria-label="Tìm kiếm" value="" data-url="admin/category_news1/index" />
              ...
              <?php if (count($news)) { ?>
                <?php foreach ($news as $k => $v) { ?>
                  <tr>
                    ...
                    <td class="align-middle">
                      <input type="number" class="update-num-cat-news1 form-control form-control-mini m-auto" min="0" value="<?= $v['num'] ?>" data-id="<?= $v['id'] ?>" data-table="<?= $configAdmin['category_news1']['table'] ?>">
                    ...
                    <td class="align-middle text-center text-md text-nowrap">
                      <a class="text-primary mr-2" href="<?= $configBase['baseUrl'] . "admin/category_news1/show?id=" . $v['id'] ?>" title="Chỉnh sửa"><i class="fas fa-edit"></i></a>
                      <div class="dropdown d-inline-block align-middle">
                        <a id="dropdownCopy" href="<?= $configBase['baseUrl'] . "admin/category_news1/duplicate?id=" . $v['id'] ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="nav-link text-success p-0 pr-2" title="Copy">
                          <i class="far fa-clone"></i>
                        </a>
                      </div>
                      <?php if (isset($configAdmin['category_news1']['soft_delete']) && $configAdmin['category_news1']['soft_delete'] === true) { ?>
                        <a href="<?= $configBase['baseUrl'] . "admin/category_news1/soft_delete?id=" . $v['id'] ?>" class="text-danger" id="delete-item" title="Xóa tạm thời">
