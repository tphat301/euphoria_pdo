  ...
  <section class="content">
    <form action="" class="form-product-list" method="POST">
      <div class="card-footer text-sm sticky-top">
        ...
        <a class="btn btn-sm bg-gradient-success text-white" data-url="<?= $configBase['baseUrl'] . "admin/category_product2/restore_all" ?>" id="restore-all" href="javascript:void()" title="Khôi phục"><i class="fa-solid fa-trash-can-arrow-up mr-2"></i>Khôi phục</a>
        <?php if (count($data) > 0) { ?>
          <a class="btn btn-sm bg-gradient-danger text-white delete-all" id="delete-all" href="javascript:void()" title="Xóa tất cả vĩnh viễn" data-url="<?= $configBase['baseUrl'] . "admin/category_product2/delete_all" ?>"><i class="far fa-trash-alt mr-2"></i>Xóa tất cả vĩnh viễn</a>
        <?php } ?>
      ...
      <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
          <h3 class="card-title">Danh sách <?= $configAdmin['category_product2']['name'] ?></h3>
        </div>
              ...
              <?php if (count($data)) { ?>
                <?php foreach ($data as $k => $v) { ?>
                  <tr>
                    ...
                    <td class="align-middle">
                      <input type="number" class="update-num-cat-product2 form-control form-control-mini m-auto" min="0" value="<?= $v['num'] ?>" data-id="<?= $v['id'] ?>" data-table="<?= $configAdmin['category_product2']['table'] ?>">
                    ...
                    ?>
                      <td class="align-middle text-center">
                        <div class="custom-control custom-checkbox my-checkbox">
                          <input type="checkbox" class="check-sst-product2 custom-control-input show-checkbox" id="check-sst-product2" data-table="<?= $configAdmin['category_product2']['table'] ?>" name="<?= $kStatus ?>" data-id="<?= $v['id'] ?>" <?= in_array($kStatus, $status) && !empty($status) ? 'checked' : '' ?> />
                          <label for="check-sst-product2" class="custom-control-label"></label>
                          <input name="_token" value="<?php echo time() ?>" type="hidden" />
                        </div>
                      </td>
                    <?php } ?>
                    <td class="align-middle text-center text-md text-nowrap">
                      <a href="javascript:void()" class="text-success" data-url="<?= $configBase['baseUrl'] . "admin/category_product2/restore?id=" . $v['id'] ?>" id="restore-item" title="Khôi phục">
