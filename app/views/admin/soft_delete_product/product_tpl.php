  ...
  <section class="content">
    <form action="" class="form-product-list" method="POST">
      <div class="card-footer text-sm sticky-top">
        ...
        <a class="btn btn-sm bg-gradient-success text-white" data-url="<?= $configBase['baseUrl'] . "admin/product/restore_all" ?>" id="restore-all" href="javascript:void()" title="Khôi phục"><i class="fa-solid fa-trash-can-arrow-up mr-2"></i>Khôi phục</a>
        <?php if (count($data) > 0) { ?>
          <a class="btn btn-sm bg-gradient-danger text-white delete-all" id="delete-all" href="javascript:void()" title="Xóa tất cả vĩnh viễn" data-url="<?= $configBase['baseUrl'] . "admin/product/delete_all" ?>"><i class="far fa-trash-alt mr-2"></i>Xóa tất cả vĩnh viễn</a>
        <?php } ?>
                  ...
                  <tr>
                    ...
                    ?>
                      <td class="align-middle text-center">
                        <div class="custom-control custom-checkbox my-checkbox">
                          <input type="checkbox" class="check-sst-product custom-control-input show-checkbox" id="<?= $kStatus ?>-check-sst-product" data-table="<?= $configAdmin['product']['table'] ?>" name="<?= $kStatus ?>" data-id="<?= $v['id'] ?>" <?= in_array($kStatus, $status) && !empty($status) ? 'checked' : '' ?> />
                          <label for="<?= $kStatus ?>-check-sst-product" class="custom-control-label"></label>
                          <input name="_token" value="<?php echo time() ?>" type="hidden" />
                        </div>
                      </td>
                    <?php } ?>
                    <td class="align-middle text-center text-md text-nowrap">
                      <a href="javascript:void()" class="text-success" data-url="<?= $configBase['baseUrl'] . "admin/product/restore?id=" . $v['id'] ?>" id="restore-item" title="Khôi phục">
